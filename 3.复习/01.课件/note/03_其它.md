...
- 3XX: 需要重定向, 浏览器直接跳转, 如302(地址重定向,如果是http请求就会控制用户跳转到某个地址上),304(协商缓存,其实就是检查用户之前是否缓存了当前文件,如果有缓存,就将请求地址重定向到用户的内存中)
- 4XX: 客户端请求错误, 如:  403(代表服务器具有该资源,但是你权限不够不给你), 404(not Found,没有该资源,在服务器上没有该资源)
- 5XX: 服务器端错误, 如: 500
...
1. 配置通用的基础路径和超时
   1. **扩展:Chrome和FireFox请求最大并发数为6个,小程序是10个**
2. 显示请求进度条
   1. 显示进度条: 请求拦截器回调
   2. 结束进度条: 响应拦截器回调
...
- Restless API  
  ...
  - 传统的API, 把每个url当作一个功能操作      /deleteUser 	/addUser
...
1. 常用手段具有两种
   1. 使用localStorage配合storage事件(发送数据的页面简称A页面,接收数据的页面简称B页面)
      1. 在B页面给window绑定事件,事件名storage,此事件可以监视localStorage的设置
      2. 在A页面使用localStorage.setItem来设置缓存数据
      3. 此时会触发B页面绑定storage事件的回调函数
      4. 最终从该事件回调函数的事件对象中可以得到A页面传递的数据
      5. 注意:**该方案只适用于两个页面都是同一个域名**
   2. 使用domain属性实现(发送数据的页面简称B页面,接收数据的页面简称A页面)
      1. 在A页面执行document.domain="公司网址的一级域名"代码
      2. 在A页面使用window.open方法打开一个全新的B页面,并且使用变量接收open方法返回的B页面的window对象(简称w1)
      3. 在B页面执行document.domain="公司网址的一级域名"代码
      4. 在B页面,通过window对象添加全局变量,并且赋值
      5. 在A页面,通过w1对象可以获取到B页面的全局变量数据
      6. 注意:**该方案只适用于两个页面具有同一个顶级域名**
   3. 注意:A与B页面必须同属于一家公司
2. 扩展方法
   1. 使用window.open打开全新页面的同时,在跳转地址中拼接query传参可以实现(params其实也可以,但是需要后端进行特殊配置)
