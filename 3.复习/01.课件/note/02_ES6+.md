...
- 默认暴露
  - export default 123;
  - 引入语法:import A from '路径'
  - 默认暴露出来的内容也是一个对象,
  - {default:123}
- 分别暴露
  - export const a = 123;
  - export const b = 666;
  - 引入语法: import { a:a } from '@/api'
  - 分别暴露出来的内容是一个对象,对象内部具有a和b属性
- 统一暴露
- export {a:123,b:666}
- 问题:请问默认暴露和分别暴露能否同时使用在一个文件中?
  - 可以
  - 在做后台管理系统配置router时候,分别暴露了3个路由数组和默认暴露一个路由器对象
  - import {anyRoutes,asyncRoutes,default:router} from '路径'
- 扩展语法:
  - 一行代码实现引入+暴露功能
...
- 扩展运算符
  - 问题:如何解释浅拷贝和深拷贝的区别?
    - const arr = [1,2,3]
    - const arr2 = arr;
    - 当开始拷贝一个对象数据类型的数据时,需要将他所有的属性值都复制到新的对象上,
    - 期间,如果发现属性值是一个对象数据类型的数据,那么此时只拷贝他的地址值就是浅拷贝,如果根据该对象创建一个长得一样的新对象,叫做深拷贝
    - 也就是说修改内部的某个对象,不会影响到旧对象,就是深拷贝,反之浅拷贝
    - 总结:如果在拷贝对象的过程中,堆内存开辟了新的空间,就是深拷贝,反之浅拷贝
...
- 静态方法
  ...
  - Array.from():  将类数组对象和可遍历对象转为真数组
    ...
    - [...new Set(arr)]
    - 旧写法:Array.prototype.slice.call(伪数组)
